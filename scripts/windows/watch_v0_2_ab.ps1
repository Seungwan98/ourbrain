...
$host.UI.RawUI.WindowTitle = 'OurBrain V0.2 A/B Training'
$experiments = @(
    [ordered]@{
        name = 'A baseline'
...
function Get-ExperimentStatus {
    ...
    if (Test-Path $exitFile) {
        ...
        $status.state = if ($code -eq 0) { 'COMPLETED' } else { "FAILED ($code)" }
    }
    elseif (Test-Path (Join-Path $runDir 'started_at.txt')) {
        $status.state = 'TRAINING'
    }
    if (Test-Path $stderr) {
        $line = Get-Content $stderr -Tail 200 |
        ...
        if ($line -match 'epoch\s+(\d+)/(\d+):\s+(\d+)%.*?\|\s*(\d+)/(\d+)') {
            ...
            $status.progress = "epoch $epoch/$epochs | step $step/$steps | $percent% | total $overall%"
        }
    }
    return $status
}
...
do {
    ...
    Write-Host '          OURBRAIN V0.2 - CONTROLLED A/B TRAINING' -ForegroundColor Cyan
    Write-Host '============================================================' -ForegroundColor DarkCyan
    if (-not (Test-Path $manifest)) {
        Write-Host 'STATUS : WAITING FOR 200/200 HUMAN REVIEW + STRICT IMPORT' `
            -ForegroundColor Yellow
        Write-Host 'Review : https://ourbrain-tunnel-review.vercel.app'
    }
    else {
        $statuses = @($experiments | ForEach-Object { Get-ExperimentStatus $_ })
        foreach ($status in $statuses) {
            ...
            Write-Host ("{0,-12}: {1}" -f $status.name, $status.state) -ForegroundColor $color
    ...
    Write-Host 'This monitor can be closed without stopping training.' -ForegroundColor DarkGray
    if ($Once) {
        break
    }
